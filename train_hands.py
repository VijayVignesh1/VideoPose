...
class PoseDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        while count!=i+num_frames:
            ...
            filtered = dict(zip(self.palms, [val_dict[k].detach().tolist() for k in self.palms]))
...
def evaluate(model, gen_input, true_data, val_data):
    """
    ...
    generated_data = model(gen_input)
    ...
    filehandler = open("output_files/output_hands_dense_no_adv.txt", 'wt')
    output = str(output)
    filehandler.write(output)


    loss = loss_model(generated_data.view(-1,num_frames, num_output_joints, num_dof), true_data)
...
def train(checkpoint=None):
    ...
    if checkpoint:
        generator.load_state_dict(torch.load(checkpoint)['model'])

    optimizer = torch.optim.Adam(model.parameters(), lr=0.0001)
    
    optimizer_scheduler = StepLR(optimizer, step_size=45, gamma=0.1)
    ...
    for epoch in range(epochs):
        ...
        for j, (x_val, values_val) in enumerate(valid_loader):
            evaluate(model, x_val.to(device), values_val.to(device), val_data)
            break
        ...
        'checkpoints/checkpoint_hands_dense_no_adv.pth.tar')
