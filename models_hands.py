...
class HandEncoder(torch.nn.Module):
    def __init__(self):
        ...
        self.T=num_frames
        self.encoder=torch.nn.Linear(self.T*num_input_joints*input_dof,self.T*self.P)
        # self.conv= nn.Conv1d(self.T*18, out_layer, kernel_size=kernel_size,bias=True)
    def forward(self,parameters):
        out=self.encoder(parameters)
        # out=self.conv(parameters)
        return out
